...
let directionFromChar = function
  | 'U' -> Up
  | 'D' -> Down
  | 'L' -> Left
  | 'R' -> Right
...
let origin = {X=0.; Y=0.}
type WireSegment = { Start: Point; End: Point; StartStepCount: int; EndStepCount: int }

type Intersection = { Point: Point; StepCount: int }

let toPathStep (x: string) : PathStep = {
  Direction = x.[0] |> directionFromChar
  Amount = x.[1..] |> Int32.Parse
}

let nextPoint (startPoint: Point) (step: PathStep) =
  let amount = float step.Amount
  match step.Direction with
  | Up -> {startPoint with Y = startPoint.Y + amount}
  | Down -> {startPoint with Y = startPoint.Y - amount}
  | Left -> {startPoint with X = startPoint.X - amount}
  | Right -> {startPoint with X = startPoint.X + amount}

let mapSegments (steps: PathStep seq) =
  let folder (state: (Point * int) list) (nextStep: PathStep) =
    let point, stepCount = state.Head
    let nextPoint = (nextPoint point nextStep)
    (nextPoint, (stepCount+nextStep.Amount)) :: state
  steps
  |> Seq.fold folder [ (origin, 0) ]
  |> Seq.rev
  |> Seq.pairwise
  |> Seq.map (fun ((p1, c1), (p2, c2)) -> {Start=p1; End=p2; StartStepCount=c1; EndStepCount=c2})

let getIntersectionPoint (firstSegment: WireSegment) (secondSegment: WireSegment) =
  let (Ax,Ay,Bx,By,Cx,Cy,Dx,Dy) = (firstSegment.Start.X, firstSegment.Start.Y, firstSegment.End.X, firstSegment.End.Y, secondSegment.Start.X, secondSegment.Start.Y, secondSegment.End.X, secondSegment.End.Y)
  let d = (Bx-Ax)*(Dy-Cy)-(By-Ay)*(Dx-Cx)
  match d with
  | 0. -> None // Parallel lines
  | d ->
